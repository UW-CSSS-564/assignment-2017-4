...
```{r}
wars <- read.dta('nwarsl.dta')
other <- read.dta('fl.dta')
...
Example:
```{r}
mod <- stan_glm(onset ~ loglang, family = binomial(), data = df)
loo_mod <- loo(mod)
mod2 <- stan_glm(onset ~ loglang + Oil, family = binomial(), data = df)
loo_mod2 <- loo(mod2)
...
- Is the default LOO method appropriate? If not, discuss an approporiate cross-validation method - (there is no unique answer)
